...
- 【2026-08-02 更新】**临时文件清理免授权**：验证后确认不需要的临时文件（如调试脚本、pytest_out.txt、tsc 输出等）可直接删除，不必每次征求授权。但用户明确拒绝删除的历史/疑似个人文件（如 root 级 tsc_out.txt、report_orig.xlsx、verify_report.txt、push_out.txt）仍保留不删。
